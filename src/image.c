...
Image negatif_image(Image I)
{
	UINT h = hauteur_image(I);
	UINT l = largeur_image(I);
	Image IMAGE_N = creer_image(l,h);
	int i,j;
	for (i=1; i<=h; i++){
		for (j=1; j<=l; j++){
			if (get_pixel_image(I,j,i) == 0)
				set_pixel_image(IMAGE_N,j ,i, 1);
			else
				set_pixel_image(IMAGE_N,j ,i, 0);				
		}
	}
	return IMAGE_N;
...
Image masque(Image I){
    ...
    for (i=1;i<=h;i++){
        for(j=1;j<=l;j++){
            if ((get_pixel_image(I,j,i-1) ==0) && (get_pixel_image(I, j, i) == 1))
                    set_pixel_image(masque, j, i, 1);
        }
